...
import { appendFile, existsSync, readFileSync, rename, stat } from "node:fs";
...
/**
 * Locate the Memory Tree engine root without hardcoded machine paths.
 * Priority: bundled engine/ inside this package, then the sibling
 * "Memory Tree System" directory (legacy dev layout). Users can override
 * with DSH_MEMORY_BRIDGE_ENGINE_ROOT or config.engineRoot.
 */
function detectEngineRoot() {
	// 1) bundled engine shipped inside the plugin package
	const bundled = join(PLUGIN_ROOT, "engine");
	if (existsSync(join(bundled, "memory"))) return bundled;
	// 2) sibling layout: <plugins>/Memory Tree System (dev workspace)
	const sibling = join(PLUGIN_ROOT, "..", "Memory Tree System");
	if (existsSync(join(sibling, "memory"))) return sibling;
	return "";
}
...
function resolveOptions(config) {
	...
	const engineRoot = fromEnv?.trim()
		...
		|| detectEngineRoot();
	const pythonExe = process.env.DSH_MEMORY_BRIDGE_PYTHON?.trim()
		|| config?.pythonExe?.trim()
		|| DEFAULT_OPTIONS.pythonExe;
	return {
		...DEFAULT_OPTIONS,
		...(config || {}),
		engineRoot,
...
/**
 * 同源校验（分严格/宽松两档）：
 * - 严格（POST，写操作）：必须带 Origin 且匹配——浏览器同源 POST fetch 必带 Origin。
 * - 宽松（GET，只读）：带 Origin 的必须匹配（防跨站读取），**无 Origin 放行**——
 *   浏览器同源 GET fetch 与地址栏访问都不带 Origin，严格校验会 403 挂掉 UI。
 */
function originOk(request, strict) {
	const origin = request.headers.origin;
...
const GET_METHODS = {
	...
	"audit": { method: "audit" },
	"graph": { method: "graph" },
	"profile-status": { method: "profileStatus" },
};
...
function mountRoutes(host, sidecar, logger) {
	...
	const POST_METHODS = {
		...
		"distill": "distill",
		"distill-approve": "distillApprove",
		"distill-reject": "distillReject",
	};
	disposers.push(host.webServer.register({
		...
		handler: async (request, response) => {
			...
			try {
				// 同源校验：POST 严格（写操作，同源必带 Origin）。
				// GET 只读但 search 带统计副作用（M6）：无 Origin 时必须携带本地标记
				// header（跨站 <img>/<script> 无法自定义 header → 挡掉状态污染）。
				if (isPost) {
					...
					const origin = request.headers.origin;
					const hasLocal = request.headers["x-dsh-memory"] === "1";
					if (!hasLocal && !(origin !== undefined && sameOrigin(request))) {
						sendJson(response, 403, { ok: false, error: "untrusted origin" });
						return;
					}
				}
...
function registerTools(ctx, sidecar, logger) {
	ctx.tools.register(defineTool({
		...
		output: {
			...
			render: (_args, value) => {
				...
				if (results.length === 0) return "(memory_search: no results)";
				const lines = results.map((r, i) => `${i + 1}. [${r.score !== undefined ? r.score.toFixed(3) : "?"}] ${r.title}${r.chainTitle ? `（链: ${r.chainTitle}）` : ""}\n   ${r.snippet || ""}`);
				return lines.join("\n");
			},
		},
	...
	ctx.tools.register(defineTool({
		...
		output: {
			...
			render: (_args, value) => `(memory_add_run: staged ${value?.runId || "?"})`,
		},
		async execute(args, exec) {
	...
	ctx.tools.register(defineTool({
		...
		output: {
			...
			render: (_args, value) => {
				...
				if (runs.length === 0) return "(memory_review: no runs)";
				const lines = runs.map((r) => `[${r.status}] ${r.runId} (${r.tier || "?"}) ${(r.userText || "").slice(0, 60)}`);
				const head = value?.staged !== undefined ? `${value.staged} staged · ` : "";
...
/* 调试日志：1MB 轮转（异步低频检查，不阻塞会话热路径 M4） */
const DEBUG_LOG = join(PLUGIN_ROOT, "memory-hook.log");
const DEBUG_MAX_BYTES = 1024 * 1024;
let _debugCount = 0;
function debugLog(line) {
	try {
		// 轮转检查异步 + 低频（每 50 条一次），避免每次同步 statSync
		if (++_debugCount % 50 === 0) {
			stat(DEBUG_LOG, (err, st) => {
				if (!err && st.size > DEBUG_MAX_BYTES) rename(DEBUG_LOG, DEBUG_LOG + ".1", () => { /* no-op */ });
			});
		}
		appendFile(DEBUG_LOG, `[${new Date().toISOString()}] ${line}\n`, () => { /* fire-and-forget */ });
...
function registerMemoryInject(ctx, sidecar, logger) {
	...
	const onUserMessage = (session, event) => {
		try {
			...
			if (!query) return;
			const sessionId = String(session.id ?? "unknown");
			void sidecar.call("inject", { sessionId, query: query.slice(0, 2000), tier: "auto" })
				.then((res) => {
					cache.set(sessionId, { text: res.text || "", cards: res.cards || [], at: Date.now(), tier: res.tier || "", rendered: false });
					if ((res.cards || []).length) {
						logger?.info?.(`[dsh-memory-bridge] injected ${res.cards.length} memory card(s) [${res.tier || "?"}] for ${sessionId.slice(0, 8)}`);
						debug(`inject ${res.cards.length} cards [${res.tier || "?"}] (${sessionId.slice(0, 8)})`);
					} else {
						debug(`inject 0 cards [${res.tier || "?"}] (${sessionId.slice(0, 8)})`);
					}
				})
				.catch((err) => debug(`inject FAILED: ${err.message}`));
			// 零 LLM 即时记忆（rules.py 设计原意）：用户消息到达即规则扫描——
			// "记住教训/踩坑"→永久经验、"记住：X"→事件、"我喜欢/习惯"→偏好信号入账。
			// 不依赖 LLM 提取（快、确定、不抢 TTFT）；只扫用户文本（偏好/指令都是用户说的）。
			void sidecar.call("recorder", { userText: query.slice(0, 2000), sessionId })
				.then((res) => {
					if (res.cards && res.cards.length) {
						logger?.info?.(`[dsh-memory-bridge] rule-recorded ${res.cards.length} card(s): ${res.cards.map(c => `${c.kind}:${c.cardId}`).join(",")}`);
						debug(`recorder cards: ${JSON.stringify(res.cards)}`);
					} else if (res.proposed && res.proposed.length) {
						logger?.info?.(`[dsh-memory-bridge] lesson proposal: ${res.proposed.length} 张（${res.proposed.map(p => p.cardId).join(",")}）`);
						debug(`lesson proposed: ${JSON.stringify(res.proposed)}`);
					} else if (res.recorded > 0) {
						debug(`preference signal recorded (${sessionId.slice(0, 8)})`);
					}
				})
				.catch((err) => debug(`recorder FAILED: ${err.message}`));
		} catch (err) {
	...
	const onTurnEnd = (session, event) => {
		try {
			...
			// M1：仅当本轮 system prompt 实际渲染了注入文本才记审计——预取竞态下
			// 模型没见过的卡若记 unused 会被治理降权（系统性误伤好卡）。
			if (!hit.rendered) {
				debug(`recordUsage skipped (not rendered) (${sessionId.slice(0, 8)})`);
				return;
			}
			let replyText = "";
			for (const ev of session.events || []) {
				...
				disposeContext = sp.context({
					...
					text: (context) => {
						try {
							...
							if (hit && hit.text) {
								hit.rendered = true;  // M1：本轮渲染了注入 → 审计闭环才生效
								return hit.text;
							}
							return "";
						} catch { return ""; }
