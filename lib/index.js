...
function registerAutoExtract(ctx, sidecar, logger) {
	...
	const stageAndExtract = async (sessionId, entry) => {
		try {
			...
			// 规则台账：扫描本轮偏好信号 → 聚合 ≥3 同类 → lesson_pending 提案
			// （沉淀经验通道；提取提示词把偏好标 explicit 直接成 event，此通道独立识别）
			try {
				const sr = await sidecar.call("recordSignals", {
					userText: entry.userText, replyText: entry.replyText,
				});
				if (sr.proposed && sr.proposed.length) {
					logger?.info?.(`[dsh-memory-bridge] lesson proposal: ${sr.proposed.length} 张（${sr.proposed.map(p => p.cardId).join(",")}）`);
					debug(`lesson proposed ${sr.proposed.length}: ${JSON.stringify(sr.proposed)}`);
				} else if (sr.recorded > 0) {
					debug(`signals recorded: ${sr.recorded}`);
				}
			} catch (err) {
				debug(`recordSignals FAILED: ${err.message}`);
			}
		} catch (err) {
