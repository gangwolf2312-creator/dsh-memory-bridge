...
**解决什么问题**：DSH 的会话能力边界（源码核实）：
- **会话持久化**：事件溯源日志 + `--resume` 恢复 / fork 派生——能力是"**续上旧对话**"，新会话默认不携带旧会话内容；
- **会话压缩**（compaction）：处理**单会话内**超长上下文的 token 压力，摘要在该会话内复用；
- **官方无记忆层**：`@deepseek-ai` 全量扫描无 memory/recall 模块——**没有从历史提取事实、跨会话注入的机制**。

本插件补的正是这一层：把对话中值得留存的（你是谁、做过什么、哪些决定被认可、哪些经验被验证）**提取、结构化、存储**，并在新会话中**按需注入**——这是 DSH 会话机制之外的**跨会话持久化事实记忆**。

> **类比**：DSH 的 resume/fork 像"翻开旧笔记本继续写同一页"；本插件像"把值得留存的整理成档案卡，之后任何新页面都能查"。两者互补，不是替代。
...
| **历史不可检索/治理**：会话日志是流水文本，DSH 无记忆层（官方无 memory 模块），不可查"我之前说过什么偏好" | 结构化记忆卡 + 确定性检索 + 分档注入 + 治理闭环 |
