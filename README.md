...
**解决什么问题**：LLM 是无状态的推理引擎，每次请求独立；DSH 提供会话内上下文与工作区，但**跨会话的持久化事实**（你是谁、做过什么、哪些决定被认可）不在其中。本插件把这些事实从对话中提取、结构化存储（事件、事件链、经验、画像、知识），并在后续对话按需注入——为 DSH 补充一个**会话之外、可检索、可治理的持久记忆层**。
...
| 痛点 | 本插件的解法 |
|---|---|
| **跨会话上下文缺失**：新会话不自动携带旧会话的持久事实（你是谁、做过什么、既定决定）| 自动提取沉淀 + 后续会话按需注入，恢复关键背景 |
